...
// CLABE interbancaria - REEMPLAZA con tu CLABE real de 18 dígitos
const CLABE_INTERBANCARIA = '000000000000000000';

// Inicialización
document.addEventListener('DOMContentLoaded', () => {
    // Leer parámetros de URL (opcionales). Ej: index.html?from=Cristian&to=WENDY
    const urlParams = new URLSearchParams(window.location.search);
    fromName = (urlParams.get('from') || '').replace(/-/g, ' ');
    toName = (urlParams.get('to') || '').replace(/-/g, ' ');
    
    // Personalizar mensajes si hay from/to; si no, se usan los textos por defecto del HTML
    personalizeMessages();
...
function personalizeMessages() {
    ...
    if (successMessage1 && fromName) {
        successMessage1.textContent = `${fromName} dice: Eres la mejor persona del mundo y me haces muy feliz.`;
    }
    
    if (successMessage2 && fromName) {
        successMessage2.textContent = `¡Te amo infinitamente ${toName}! 💖`;
    }
}
...
function setupEventListeners() {
    ...
    if (questionContainer) questionContainer.addEventListener('mousemove', (e) => {
